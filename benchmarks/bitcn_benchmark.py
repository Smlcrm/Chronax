"""BiTCN comparison benchmark: Chronax vs Nixtla neuralforecast.

Run with:
    .venv/bin/python benchmarks/bitcn_benchmark.py
...
(JAX/JIT vs PyTorch-Lightning), not a weakened reference. BiTCN is univariate
(no exogenous inputs on either side); ``kernel_size=2`` is fixed in NF.

Outputs (NOT committed — kept local for review):
    benchmarks/benchmark_results/bitcn_<timestamp>.csv
    benchmarks/benchmark_results/bitcn_<timestamp>_summary.csv
"""
...
from chronax.models import BiTCN
...
# Model hyperparameters — identical on both sides. Defaults follow NF BiTCN.
HIDDEN_SIZE = 16
DROPOUT = 0.1
MAX_STEPS = 1000
LEARNING_RATE = 1e-3
...
def run_chronax(spec, seed):
    ...
    model = BiTCN(
        h=H, input_size=INPUT_SIZE, hidden_size=HIDDEN_SIZE, dropout=DROPOUT,
...
m = BiTCN(h={H}, input_size={INPUT_SIZE},
...
def main():
    ...
    df.to_csv(RESULTS_DIR / f"bitcn_{ts}.csv", index=False)
    summary_total.join(after).to_csv(RESULTS_DIR / f"bitcn_{ts}_summary.csv")
    print(f"\nWrote: bitcn_{ts}.csv  bitcn_{ts}_summary.csv")

"""KAN comparison benchmark: Chronax vs Nixtla neuralforecast.

Run with (rows are written incrementally so it is crash-safe):
    .venv/bin/python benchmarks/kan_benchmark.py --resume

The Nixtla side runs in benchmarks/.venv-nf at KAN's native defaults
(scaler_type='identity', max_steps=1000, windows_batch_size=1024). The Chronax
side uses the identity scaler too (KAN's default), so the comparison is faithful.
Reports total wall-clock and after-warmup wall-clock (first seed per dataset
excluded); acceptance uses after-warmup time. Each (library, dataset, seed) row is
appended as it finishes; --resume skips rows already present.

Outputs (stable names so --resume works):
    benchmarks/benchmark_results/kan_results.csv
    benchmarks/benchmark_results/kan_results_summary.csv
"""
...
def run_chronax(spec, seed):
    ...
    model = KAN(h=H, input_size=INPUT_SIZE, max_steps=MAX_STEPS, random_seed=seed)
...
m = KAN(h={H}, input_size={INPUT_SIZE}, max_steps={MAX_STEPS}, scaler_type='identity',
        learning_rate=1e-3, random_seed={seed}, loss=MAE(),
...
def main():
    ...
    ap.add_argument("--out", default=str(RESULTS_DIR / "kan_results.csv"),

"""VanillaTransformer comparison benchmark: Chronax vs Nixtla neuralforecast.

Run with:
    .venv/bin/python benchmarks/vanillatransformer_benchmark.py
...
(JAX/JIT vs PyTorch-Lightning), not a weakened reference. Chronax ``n_heads``
maps to NF ``n_head``.

Outputs (NOT committed — kept local for review):
    benchmarks/benchmark_results/vanillatransformer_<timestamp>.csv
    benchmarks/benchmark_results/vanillatransformer_<timestamp>_summary.csv
"""
...
from chronax.models import VanillaTransformer
...
# Model hyperparameters — identical on both sides. Defaults follow NF VanillaTransformer.
HIDDEN_SIZE = 128
N_HEADS = 4
CONV_HIDDEN_SIZE = 32
ENCODER_LAYERS = 2
DECODER_LAYERS = 1
DROPOUT = 0.05
MAX_STEPS = 1000           # reduced from NF's 5000 for a fast warm-start benchmark
LEARNING_RATE = 1e-4
...
def run_chronax(spec, seed):
    ...
    model = VanillaTransformer(
        ...
        conv_hidden_size=CONV_HIDDEN_SIZE, encoder_layers=ENCODER_LAYERS,
        decoder_layers=DECODER_LAYERS, dropout=DROPOUT, use_boxcox=USE_BOXCOX,
...
m = VanillaTransformer(h={H}, input_size={INPUT_SIZE},
                 hidden_size={HIDDEN_SIZE}, n_head={N_HEADS},
                 conv_hidden_size={CONV_HIDDEN_SIZE}, encoder_layers={ENCODER_LAYERS},
...
y_hat = fcst['VanillaTransformer'].to_numpy()
eps = 1e-10
...
def main():
    ...
    print(f"config: input_size={INPUT_SIZE} max_steps={MAX_STEPS} hidden_size={HIDDEN_SIZE} "
          f"wbs={WINDOWS_BATCH_SIZE} lr={LEARNING_RATE} use_boxcox={USE_BOXCOX} seeds={SEEDS}")
    ...
    df.to_csv(RESULTS_DIR / f"vanillatransformer_{ts}.csv", index=False)
    summary_total.join(after).to_csv(RESULTS_DIR / f"vanillatransformer_{ts}_summary.csv")
    print(f"\nWrote: vanillatransformer_{ts}.csv  vanillatransformer_{ts}_summary.csv")

"""BiTCN forecaster: BaseForecaster wrapper around the JAX/Flax-NNX backbone."""
...
from chronax.models.bitcn.bitcn_losses import LossFn, resolve as _resolve_loss
from chronax.models.bitcn.bitcn_module import BiTCNNet
from chronax.models.bitcn.bitcn_training import predict_step, train
from chronax.utils import ConformalIntervals

# --- Box-Cox variance-stabilizing transform (self-contained) ---------------
# Mirrors the ``use_boxcox`` convention of chronax.models.TBATS / VanillaTransformer.
...
class BiTCN(BaseForecaster):
    ...
    Bidirectional Temporal Convolutional Network (Sprangers, Schelter & de Rijke,
    2023). For the univariate, no-exogenous case only the *backward* dilated TCN
    is used: the lookback window is projected to ``hidden_size``, passed through
    ``ceil(log2(input_size))`` dilated causal-conv cells (``kernel_size=2``,
    dilation ``2**i``) accumulating a residual state and a skip signal, then two
    temporal dense layers map to the horizon and a final linear projects to one
    channel. NF default ``scaler_type="identity"`` is mirrored (trained in raw
    ...
    def __init__(
        ...
        """Initialize a BiTCN forecaster.

        Stores hyperparameters; the network is built lazily at ``fit`` time.
        Defaults match neuralforecast.BiTCN (``hidden_size=16``, ``dropout=0.5``,
        ``max_steps=1000``, ``learning_rate=1e-3``, ``windows_batch_size=1024``,
        ``scaler_type="identity"``, ``kernel_size=2`` fixed). ``input_size=-1``
        resolves to ``3 * h``. ``loss`` is a registry name
        (``"mae"``/``"mse"``/``"huber"``) or a callable; ``learning_rate`` is a
        scalar or an ``optax.ScalarOrSchedule``. If the fitted estimator will be
        pickled, any callable passed for ``loss``/``learning_rate`` must itself be
        picklable.
        ...
        Left off, the model is a faithful port of neuralforecast.BiTCN.
        """
        if input_size < 1:
            input_size = 3 * h
        self.h = h
        ...
        self.model_: BiTCNNet | None = None
    ...
    def _build_net(self) -> BiTCNNet:
        return BiTCNNet(
            h=self.h, input_size=self.input_size, hidden_size=self.hidden_size,
            dropout=self.dropout, rngs=nnx.Rngs(self.random_seed),
        )

    def fit(self, y: jnp.ndarray, X: jnp.ndarray | None = None) -> "BiTCN":
    ...
    def predict(
        ...
        if h > self.h:
            raise ValueError(
                f"BiTCN was trained for h={self.h}; predict(h={h}) is not supported. "
        ...
        if level is not None:
            if self.conformal_params is None:
                raise ValueError(
                    ...
                    "BiTCN this is expensive — expect MINUTES per call."
                )
    ...
    def __getstate__(self) -> dict:
        ...
        BiTCN uses only ``nnx.Param`` variables (no BatchNorm), so the full state
        split captures exactly the parameters.
        """

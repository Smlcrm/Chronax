"""Window construction and training/predict steps for BiTCN.
...
``BaseForecaster.conformity_scores``.
"""
...
from chronax.models.bitcn.bitcn_losses import LossFn, mae
...
def forward_loss(model: BiTCNNet, windows: jnp.ndarray, *, h: int,
...
@nnx.jit
def _jit_forward_deterministic(model: BiTCNNet, x: jnp.ndarray) -> jnp.ndarray:
    """Inference-mode forward (dropout disabled). Cached across calls."""
    return model(x, deterministic=True)


def train(model: BiTCNNet, y: jnp.ndarray, *, h: int, input_size: int,
...
def predict_step(model: BiTCNNet, y: jnp.ndarray, *, h: int, input_size: int) -> jnp.ndarray:

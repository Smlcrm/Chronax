...
from chronax.utils import ConformalIntervals,_demand, _intervals_c, _ses_forecast_nan, _repeat_val, _expand_fitted_demand, _expand_fitted_intervals
...
def _croston_classic(
    ...
    return jax.lax.cond(
        # yd.size == 0, # Incorrect: `yd` is a static-sized JAX array (padded with NaNs), so `yd.size` is never 0.
        # jnp.all(y == 0),  # Incorrect: This would fail for series with negatives (e.g., [0, -5, 0]), which also lack positive demand.
        jnp.all(y <= 0),
        no_demand_forecast,
        croston_forecast
    )

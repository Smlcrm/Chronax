"""Window construction and training/predict steps for DeepNPTS.

Windows are sampled *inside* the ``nnx.scan`` (the per-step PRNG key is the
scanned input), so memory stays bounded to one batch even at NF defaults
(``max_steps=1000``, ``windows_batch_size=1024``). The whole loop is one
``nnx.scan`` so it stays ``jax.vmap``-traceable for
``BaseForecaster.conformity_scores``. When ``batch_norm`` is enabled the carried
model's ``nnx.BatchStat`` running statistics update in place through the scan.
"""
...
from chronax.models.deepnpts.deepnpts_losses import LossFn, mae
...
def forward_loss(model: DeepNPTSNet, windows: jnp.ndarray, *, h: int,
...
@nnx.jit
def _jit_forward_deterministic(model: DeepNPTSNet, x: jnp.ndarray) -> jnp.ndarray:
    """Inference-mode forward (dropout disabled, BatchNorm uses running stats)."""
    return model(x, deterministic=True)


def train(model: DeepNPTSNet, y: jnp.ndarray, *, h: int, input_size: int,
...
def predict_step(model: DeepNPTSNet, y: jnp.ndarray, *, h: int, input_size: int) -> jnp.ndarray:

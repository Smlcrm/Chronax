"""Window construction and JIT-compiled training/predict steps for DLinear."""
...
from chronax.models.dlinear.dlinear_losses import LossFn, mae
from chronax.models.dlinear.dlinear_module import DLinearNet
from chronax.models.dlinear.dlinear_scaler import Scaler
...
def scaled_forward_loss(model: DLinearNet, windows: jnp.ndarray, mask: jnp.ndarray, *, h: int,
...
@nnx.jit
def _jit_forward(model: DLinearNet, x: jnp.ndarray) -> jnp.ndarray:
    return model(x)
...
def _sample_batch_idx(step_keys: jnp.ndarray, n_windows: int, windows_batch_size: int) -> jnp.ndarray:
    ...
    model-independent, and DLinear adds only a second matmul plus a moving
    average per step). Under a trace of the keys
...
def train(model: DLinearNet, y: jnp.ndarray, *, h: int, input_size: int, max_steps: int,
...
def predict_step(model: DLinearNet, y: jnp.ndarray, *, h: int, input_size: int, scaler: Scaler) -> jnp.ndarray:

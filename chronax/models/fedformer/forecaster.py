"""FEDformer forecaster: BaseForecaster wrapper around the Flax Linen backbone.

Univariate Frequency Enhanced Decomposed Transformer matching neuralforecast.FEDformer
for the no-exogenous case. Training uses rolling windows with per-window RobustScaler;
...
from chronax.models.fedformer.loss import LossFn, resolve as _resolve_loss
from chronax.models.fedformer.model import FEDformerConfig
from chronax.models.fedformer.train import TrainState, predict_step, train
...
class FEDformer(BaseForecaster):
    ...
    Maintenance Status:
        ...
        ``predict(level=...)``. Defaults match neuralforecast.FEDformer architecture
        (``n_heads`` must be 8); ``input_size=-1`` resolves to ``3 * h``.
    """
    ...
    def __init__(
        ...
        self.fea_activation = fea_activation
    ...
    def _build_config(self) -> FEDformerConfig:
    ...
    def fit(self, y: jnp.ndarray, X: jnp.ndarray | None = None, *, futr_exog=None) -> "FEDformer":
        ...
        if X is not None:
            raise NotImplementedError(
                "FEDformer supports future-known exog only; pass futr_exog="
            )
    ...
    def _raw_predict(self, futr_exog=None) -> jnp.ndarray:
        ...
        if self._futr_size > 0:
            if futr_exog is None:
                raise ValueError(
                    "This FEDformer was fit with future-known exog; predict requires "
    ...
    def predict(
        ...
        *,
        ...
        if h > self.h:
            raise ValueError(
                f"FEDformer was trained for h={self.h}; predict(h={h}) is not supported. "
    ...
    def forecast(
        ...
        *,
        ...
        if X is not None:
            raise NotImplementedError(
                "FEDformer supports future-known exog only; pass futr_exog= / X_future="
            )
...
class FEDformerForecaster(FEDformer):
    ...
    def __init__(
        ...
        *,
        ...
        warnings.warn(
            "FEDformerForecaster(config=...) is deprecated; "
            "use FEDformer(h=..., input_size=..., random_seed=...) instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        super().__init__(
            ...
            fea_activation=config.fea_activation,
            max_steps=max_steps,

...
from chronax.models.itransformer.itransformer_losses import LossFn, mae
...
def forward_loss(model: ITransformerNet, windows: jnp.ndarray, *, h: int,
...
@nnx.jit
def _jit_forward_deterministic(model: ITransformerNet, x: jnp.ndarray) -> jnp.ndarray:
    """Inference-mode forward (dropout disabled). Cached across calls."""
    return model(x, deterministic=True)


def train(model: ITransformerNet, y: jnp.ndarray, *, h: int, input_size: int,
    ...
    tensor up front. At iTransformer's defaults (windows_batch_size=32) this is small;
...
def predict_step(model: ITransformerNet, y: jnp.ndarray, *, h: int, input_size: int) -> jnp.ndarray:

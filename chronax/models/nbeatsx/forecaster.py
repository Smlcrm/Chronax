"""High-level fit/forecast adapter around Chronax N-BEATSx.

Mirrors ``neuralforecast.NeuralForecast(models=[NBEATSx(...)]).fit(df).predict()``
...
``train_batch_step`` / ``eval_batch_step`` / ``create_batch`` API from this
package — the same trade-off already established by
:class:`~chronax.models.tide.forecaster.TiDEForecaster`. To use the
``"exogenous"`` block type, construct :class:`~chronax.models.nbeatsx.model.NBEATSx`
directly with a config carrying ``futr_exog_size`` / ``hist_exog_size`` /
``stat_exog_size`` and drive training with
:func:`chronax.models.nbeatsx.train.train_batch_step`.
"""
...
from chronax.models.nbeatsx.loss import LossFn
from chronax.models.nbeatsx.loss import resolve as _resolve_loss
...
class NBEATSxForecaster(BaseForecaster):
    """High-level fit/forecast wrapper around :class:`~chronax.models.nbeatsx.model.NBEATSx`.

    Args:
        h: Forecast horizon.
        input_size: History window length; -1 (default) uses ``3 * h``.
        stack_types, n_blocks, mlp_units, mlp_layers, n_harmonics, n_basis,
        basis, activation, shared_weights, dropout_prob, layer_norm,
        futr_exog_size, hist_exog_size, stat_exog_size: forwarded to
        :class:`NBEATSxConfig`. The window-based :meth:`fit` / :meth:`forecast`
        ...
        loss: Registered name (``"mae"``, ``"mse"``) from
            :mod:`chronax.models.nbeatsx.loss` or a callable ``(y, y_hat) -> scalar``.
    ...
    def state(self) -> TrainState:
        if self._state is None:
            raise RuntimeError("NBEATSxForecaster has not been fit yet.")
        return self._state
    ...
    def fit(
        ...
        *,
        ...
        """Train the model in-place on ``y`` (no exogenous covariates).
    ...
    def predict(
        ...
        if level is not None:
            raise NotImplementedError(
                "NBEATSxForecaster.predict(level=...) is not yet supported."
            )
    ...
    def with_config(self, **overrides) -> "NBEATSxForecaster":
    ...
    def __repr__(self) -> str:
        return (
            f"NBEATSxForecaster(config={asdict(self.config)}, "

"""Loss functions for Chronax N-BEATSx.

Mirrors the RNN / NBEATS loss API:
  masked_mae(y, y_hat, mask=None) -> scalar
  masked_mse(y, y_hat, mask=None) -> scalar

Shapes follow the window-based training convention: ``y`` and ``y_hat`` are
``[B, h]`` (no trailing feature dim). The optional ``mask`` broadcasts from
any shape compatible with ``[B, h]``.
"""

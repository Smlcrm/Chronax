"""Window construction and JIT-compiled training/predict steps for RMoK."""
...
from chronax.models.rmok.rmok_losses import LossFn, mae
from chronax.models.rmok.rmok_module import RMoKNet
from chronax.models.rmok.rmok_scaler import Scaler
...
def scaled_forward_loss(model: RMoKNet, windows: jnp.ndarray, mask: jnp.ndarray, *, h: int,
    ...
    ``deterministic=False`` (training) enables dropout and updates the WaveKAN
    BatchNorm running stats; ``True`` uses running stats (inference).
    """
    ...
    pred = model(insample_z[..., None], deterministic=deterministic)
    return loss_fn(pred[..., 0], target_z, out_mask)


@nnx.jit
def _jit_forward(model: RMoKNet, x: jnp.ndarray) -> jnp.ndarray:
...
def train(model: RMoKNet, y: jnp.ndarray, *, h: int, input_size: int, max_steps: int,
...
def predict_step(model: RMoKNet, y: jnp.ndarray, *, h: int, input_size: int, scaler: Scaler) -> jnp.ndarray:

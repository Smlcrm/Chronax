...
from chronax.models.softs.softs_losses import LossFn, resolve as _resolve_loss
from chronax.models.softs.softs_module import SOFTSNet
from chronax.models.softs.softs_training import predict_step, train
...
class SOFTS(BaseForecaster):
    """Univariate SOFTS forecaster (JAX/Flax-NNX port of neuralforecast.SOFTS).
    ...
    SOFTS (Han et al. 2024, "SOFTS: Efficient Multivariate Time Series Forecasting
    with Series-Core Fusion") shares iTransformer's inverted encoder — each variate's
    lookback window is embedded into a token (``Linear(input_size -> hidden_size)``) —
    but replaces self-attention with the STAD (STar Aggregate-Dispatch) module: the
    series are fused into a single global *core* (``d_core``) and the core is
    dispatched back onto each series. This makes the encoder O(C) in the number of
    series instead of attention's O(C^2). RevIN-style per-window mean/std
    normalization (``use_norm``) is applied inside the network and inverted on the
    output. Trained in original scale with Optax ``adam`` and a pluggable point loss.
    ``float32`` throughout. The series is univariate (``n_series = 1``), so the encoder
    operates on a single token; the backbone is written N-generically to allow a
    future multivariate path.
    """
    ...
    def __init__(
        ...
        neuralforecast.SOFTS (``hidden_size=512``, ``d_core=512``, ``e_layers=2``,
        ``d_ff=2048``, ``dropout=0.1``, ``use_norm=True``, ``max_steps=1000``,
        ``learning_rate=1e-3``, ``windows_batch_size=32``). ``input_size=-1``
        resolves to ``3 * h``. ``loss`` is a registry name (``"mae"``/``"mse"``/
        ``"huber"``) or a callable; ``learning_rate`` is a scalar or an
        ``optax.ScalarOrSchedule``; ``activation`` is ``"gelu"`` or ``"relu"``.
        ...
        the model is a faithful port of neuralforecast.SOFTS.
        """
        if input_size < 1:
            input_size = 3 * h
        self.h = h
        ...
        self.model_: SOFTSNet | None = None
    ...
    def fit(self, y: jnp.ndarray, X: jnp.ndarray | None = None) -> "SOFTS":
        ...
        Returns:
            SOFTS: ``self``, with ``model_`` populated.
    ...
    def predict(
        ...
        if h > self.h:
            raise ValueError(
                f"SOFTS was trained for h={self.h}; predict(h={h}) is not supported. "
        ...
        if level is not None:
            if self.conformal_params is None:
                raise ValueError(
                    ...
                    "SOFTS this is expensive — expect MINUTES per call, scaling "
    ...
    def __getstate__(self) -> dict:
        ...
        (SOFTS uses LayerNorm, not BatchNorm, so there are no running stats —

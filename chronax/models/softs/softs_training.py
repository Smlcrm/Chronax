...
The training loop is identical to the iTransformer port: STAD carries its own
``nnx.Rngs`` reference, so its train-time multinomial pooling key is threaded
through the ``nnx.scan`` carry automatically (same mechanism as ``nnx.Dropout``);
no extra key plumbing is needed in ``train`` beyond the window-sampling seed.
"""
...
from chronax.models.softs.softs_losses import LossFn, mae
...
def forward_loss(model: SOFTSNet, windows: jnp.ndarray, *, h: int,
...
@nnx.jit
def _jit_forward_deterministic(model: SOFTSNet, x: jnp.ndarray) -> jnp.ndarray:
    """Inference-mode forward (dropout disabled, deterministic core). Cached across calls."""
    return model(x, deterministic=True)


def train(model: SOFTSNet, y: jnp.ndarray, *, h: int, input_size: int,
    ...
    STAD's stochastic pooling and the dropout layers both draw from the model's
    ``nnx.Rngs``; because the model is the scan carry, those key streams advance per
    step without any explicit threading here.

    Note: ``batches`` materializes a ``[max_steps, windows_batch_size, input_size+h]``
    tensor up front. At SOFTS's defaults (windows_batch_size=32) this is small; if you
    raise ``windows_batch_size`` substantially, reduce ``max_steps`` or sample
...
def predict_step(model: SOFTSNet, y: jnp.ndarray, *, h: int, input_size: int) -> jnp.ndarray:

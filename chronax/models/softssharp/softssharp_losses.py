"""Pluggable point-loss functions for the SOFTSSharp forecaster.

Self-contained (no dependency on other models' loss modules), matching the
per-package convention used across the neural ports. Each loss has signature
``(pred, target) -> scalar`` and reduces by mean over all elements. Functions are
module-level so they pickle cleanly when stored as the ``loss`` attribute of a
fitted :class:`chronax.models.softssharp.SOFTSSharp`.
"""

"""SOFTSSharp forecaster: BaseForecaster wrapper around the JAX/Flax-NNX backbone."""
...
from chronax.models.softssharp.softssharp_losses import LossFn, resolve as _resolve_loss
from chronax.models.softssharp.softssharp_module import SOFTSSharpNet
from chronax.models.softssharp.softssharp_training import predict_step, train
from chronax.utils import ConformalIntervals

# --- Box-Cox variance-stabilizing transform (self-contained) ---------------
# Mirrors the ``use_boxcox`` convention of chronax.models.TBATS and the sibling
# SOFTS port: stabilize the variance of multiplicative / strongly-trending series
# before modelling, then invert on the output. ``lambda = 0`` is the log
# transform. The lambda is selected once at fit time by maximizing the Box-Cox
# profile log-likelihood (the standard MLE), kept self-contained so this module
# does not depend on another model's internals.
...
class SOFTSSharp(BaseForecaster):
    ...
    SOFTSSharp (SOFTS#) extends SOFTS (Han et al. 2024, "SOFTS: Efficient Multivariate
    Time Series Forecasting with Series-Core Fusion") with two regularizers inside the
    STAD (STar Aggregate-Dispatch) series-core fusion block:

    * a **stochastic variable-position encoding** — a fixed sinusoidal table indexed by
      the *variate* position, scaled by a learnable ``pe_scale`` and applied with
      probability ``pe_keep_prob`` during training, then applied at its expectation
      (``pe_keep_prob * pe_scale``) at inference; and
    * **three extra dropout layers** inside STAD (after the set-FFN activation, on the
      dispatched core, and after the fusion activation).

    The rest is SOFTS unchanged: iTransformer's inverted encoder (each variate's lookback
    window is embedded into a token via ``Linear(input_size -> hidden_size)``) with STAD
    in place of self-attention, keeping the encoder O(C) in the number of series instead
    of attention's O(C^2). RevIN-style per-window mean/std normalization (``use_norm``) is
    applied inside the network and inverted on the output. Trained in original scale with
    Optax ``adam`` and a pluggable point loss. ``float32`` throughout.

    The series is univariate (``n_series = 1``), so the encoder operates on a single
    token and only row 0 of the position table is ever read; the backbone is written
    N-generically to allow a future multivariate path, where the position encoding starts
    to distinguish series.
    """
    ...
    def __init__(
        ...
        random_seed: int = 1,
        alias: str = "SOFTSSharp",
        loss: Union[str, LossFn] = "mae",
    ):
        """Initialize a SOFTSSharp forecaster.
        ...
        neuralforecast.SOFTSSharp (``hidden_size=512``, ``d_core=512``, ``e_layers=2``,
        ``d_ff=2048``, ``dropout=0.1``, ``pe_keep_prob=0.5``, ``use_norm=True``,
        ...
        ``pe_keep_prob`` is the probability of applying the variable-position encoding
        during training; at inference the encoding is scaled by this value instead.
        ``pe_keep_prob=0.0`` disables the encoding entirely in both modes, which
        reduces the block to plain SOFTS-with-extra-dropout.
        ...
        mirroring the ``use_boxcox`` option of :class:`chronax.models.TBATS` and the
        sibling :class:`chronax.models.SOFTS`. The lambda is selected once at fit time
        by maximizing the Box-Cox profile log-likelihood. It helps multiplicative /
        strongly-trending series (e.g. airline passengers) and **requires strictly
        positive values**. Left off, the model is a faithful port of
        neuralforecast.SOFTSSharp.
        """
        if input_size < 1:
            input_size = 3 * h
        ...
        self.model_: SOFTSSharpNet | None = None
    ...
    def _build_net(self) -> SOFTSSharpNet:
        return SOFTSSharpNet(
            h=self.h, input_size=self.input_size, hidden_size=self.hidden_size,
            d_core=self.d_core, e_layers=self.e_layers, d_ff=self.d_ff,
            dropout=self.dropout, use_norm=self.use_norm,
            pe_keep_prob=self.pe_keep_prob, activation=self.activation,
            rngs=nnx.Rngs(self.random_seed),
        )

    def fit(self, y: jnp.ndarray, X: jnp.ndarray | None = None) -> "SOFTSSharp":
        ...
        Returns:
            SOFTSSharp: ``self``, with ``model_`` populated.
    ...
    def predict(
        ...
        if h > self.h:
            raise ValueError(
                f"SOFTSSharp was trained for h={self.h}; predict(h={h}) is not supported. "
        ...
        if level is not None:
            if self.conformal_params is None:
                raise ValueError(
                    ...
                    "SOFTSSharp this is expensive — expect MINUTES per call, scaling "
    ...
    def __getstate__(self) -> dict:
        ...
        (SOFTSSharp uses LayerNorm, not BatchNorm, so there are no running stats —
        only ``nnx.Param`` variables are captured, including the learnable
        ``pe_scale`` gain.)
        """

"""Per-window scalers for TCN: robust (NF's TCN default) and identity.
...
class RobustScaler:
    ...
    *computes* ``masked_median(|x - median|)`` — the MEDIAN absolute deviation —
    ...
    adds eps. This sequence mirrors that code path exactly (a 2026-07-17
    "mean-AD parity fix" based on the docstring was wrong and reverted; the
    lockstep-replay loss offset it caused confirmed the code reading).
    """

"""Per-window scalers for TimesNet: identity, robust (median/MAD), standard (NF default).
...
class StandardScaler:
    """Per-window biased mean/std scaler. Matches neuralforecast's
    scaler_type='standard' (`std_statistics`): ddof=0 std over the insample,
    zeros replaced by 1.0 BEFORE adding eps (same order as RobustScaler)."""

    def stats(self, x: jnp.ndarray, axis: int = 1) -> tuple[jnp.ndarray, jnp.ndarray]:
        mean = jnp.mean(x, axis=axis, keepdims=True)
        std = jnp.sqrt(jnp.mean((x - mean) ** 2, axis=axis, keepdims=True))
        scale = jnp.where(std == 0.0, 1.0, std) + _EPS
        return mean, scale
...
def resolve_scaler(scaler: "str | Scaler") -> Scaler:
    """Resolve a scaler from a name (``'identity'``/``'robust'``/``'standard'``) or a Scaler instance."""
    if isinstance(scaler, str):
        ...
        raise ValueError(f"Unknown scaler {scaler!r}. Available: 'identity', 'robust', 'standard'.")
    return scaler

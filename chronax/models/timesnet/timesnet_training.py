"""Window construction and JIT-compiled training/predict steps for TimesNet."""
...
from chronax.models.timesnet.timesnet_losses import LossFn, mae
from chronax.models.timesnet.timesnet_module import TimesNetNet
from chronax.models.timesnet.timesnet_scaler import Scaler
...
def scaled_forward_loss(model: TimesNetNet, windows: jnp.ndarray, mask: jnp.ndarray, *, h: int,
                        input_size: int, scaler: Scaler, loss_fn: LossFn = mae,
                        dropout_key: "jnp.ndarray | None" = None) -> jnp.ndarray:
    ...
    ``dropout_key`` activates the model's embedding dropout (training only).
    """
    ...
    pred = model(insample_z[..., None], dropout_key=dropout_key)
    return loss_fn(pred[..., 0], target_z, out_mask)


@nnx.jit
def _jit_forward(model: TimesNetNet, x: jnp.ndarray) -> jnp.ndarray:
    return model(x)
...
def train(model: TimesNetNet, y: jnp.ndarray, *, h: int, input_size: int, max_steps: int,
    ...
    key_sample, key_drop = jax.random.split(jax.random.PRNGKey(seed))
    step_keys = jax.random.split(key_sample, max_steps)
    dropout_keys = jax.random.split(key_drop, max_steps)
    ...
    def step(carry, idx, dkey):
        model, opt = carry
        batch_windows = windows[idx]
        batch_mask = mask[idx]
        loss, grads = nnx.value_and_grad(
            lambda m: scaled_forward_loss(m, batch_windows, batch_mask, h=h, input_size=input_size,
                                          scaler=scaler, loss_fn=loss_fn, dropout_key=dkey)
        )(model)
        opt.update(grads)
        return (model, opt), loss

    _, losses = step((model, optimizer), batch_idx, dropout_keys)
...
def predict_step(model: TimesNetNet, y: jnp.ndarray, *, h: int, input_size: int, scaler: Scaler) -> jnp.ndarray:

"""Masked point-loss functions for the Chronax TSMixerx model.

Pure JAX implementations of MAE and MSE with multiplicative masking, JIT- and
grad-compatible. Shapes follow the ``[B, H, N]`` convention produced by
TSMixerx (batch, horizon, n_series), but the functions are generic and work
for any matching broadcast-compatible shapes.

These mirror the loss functions in :mod:`chronax.models.tsmixer.loss`; kept
as a separate copy so each model sub-package is self-contained.
"""

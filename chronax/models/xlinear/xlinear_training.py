"""Window construction and JIT-compiled training/predict steps for XLinear."""
...
from chronax.models.xlinear.xlinear_losses import LossFn, mae
from chronax.models.xlinear.xlinear_module import XLinearNet
from chronax.models.xlinear.xlinear_scaler import Scaler
...
def scaled_forward_loss(model: XLinearNet, windows: jnp.ndarray, mask: jnp.ndarray, *, h: int,
...
@nnx.jit
def _jit_forward(model: XLinearNet, x: jnp.ndarray) -> jnp.ndarray:
    return model(x)
...
def train(model: XLinearNet, y: jnp.ndarray, *, h: int, input_size: int, max_steps: int,
...
def predict_step(model: XLinearNet, y: jnp.ndarray, *, h: int, input_size: int, scaler: Scaler) -> jnp.ndarray:

...
| :--- | :--- | :--- |
| `--config` | Path to the YAML configuration file. | `--config eval_dev/config.yaml` |
...
```bash
python eval_dev/run_benchmark.py --config eval_dev/config.yaml --model AutoETS
```

**Run on a specific external dataset:**
```bash
python eval_dev/run_benchmark.py --config eval_dev/config.yaml --dataset "C:/path/to/data.csv"
```

**Generate Forecast Data (CSV):**
*Note: Plotting is currently disabled in the code.*
```bash
python eval_dev/run_benchmark.py --config eval_dev/config.yaml --forecast
```

"""Tests for chronax.models.dlinear.dlinear_model — BaseForecaster conformance."""
import pickle
...
def _tiny(**kw):
    return DLinear(h=12, input_size=36, max_steps=20, windows_batch_size=64, random_seed=0, **kw)


def test_dlinear_inherits_baseforecaster():
    assert issubclass(DLinear, BaseForecaster)
...
def test_unknown_scaler_raises_at_fit():
    with pytest.raises(ValueError, match="Unknown scaler"):
        DLinear(h=12, input_size=36, max_steps=2, windows_batch_size=8, random_seed=0, scaler="nope").fit(_make_y())


def test_even_moving_avg_window_raises():
    with pytest.raises(ValueError, match="odd"):
        DLinear(h=12, input_size=36, moving_avg_window=4)


def test_nonpositive_moving_avg_window_raises():
    with pytest.raises(ValueError, match="positive"):
        DLinear(h=12, input_size=36, moving_avg_window=-3)


def test_moving_avg_window_one_fits_and_predicts():
    m = DLinear(h=12, input_size=36, moving_avg_window=1, max_steps=5,
                windows_batch_size=16, random_seed=0).fit(_make_y())
...
def test_model_beats_naive_on_easy_signal():
    ...
    m = DLinear(h=12, input_size=36, max_steps=200, windows_batch_size=64, random_seed=0,
...
def test_h_equals_one():
    m = DLinear(h=1, input_size=12, max_steps=5, windows_batch_size=16, random_seed=0)
    m.fit(_make_y(60))
...
def test_fitted_values_match_hand_computed():
    y = _make_y(60)
    m = _tiny().fit(y)
    # zero weights + biases bt=0.3, bs=0.2: every forecast step = 0.5 exactly
...
def test_predict_with_level_returns_interval_keys():
    m = DLinear(h=4, input_size=12, max_steps=2, windows_batch_size=16, random_seed=0)
    m.fit(_make_y(80))
...
def test_dlinear_importable_from_models_namespace():
    from chronax.models import DLinear as D
    assert D is DLinear


def test_dlinear_auto_discovered_by_benchmark_harness():
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
    from benchmarks.neural import registry
    assert "DLinear" in registry.list_models()
    assert registry.resolve_chronax("DLinear") is DLinear

"""Tests for chronax.models.SOFTS.

Covers the four source modules of the softs subpackage in one file (losses,
module, training, model), matching the flat ``test_<model>.py`` convention used
elsewhere in ``tests/``. SOFTS shares iTransformer's inverted encoder skeleton but
swaps self-attention for the STAD series-core fusion module, so the module-level
tests here focus on STAD's train/eval pooling behaviour.
"""
import math
import pickle
...
from chronax.models.softs.softs_losses import (
    LOSSES, huber, mae, mse, resolve,
)
from chronax.models.softs.softs_module import (
...
from chronax.models.softs.softs_model import (
    SOFTS, _boxcox, _inv_boxcox, _select_boxcox_lambda,
)
from chronax.models.softs.softs_training import (
...
def test_revin_subtract_mean_centers_on_mean():
    # SOFTS uses the per-window MEAN (subtract_last=False), not the last value.
    revin = RevIN(num_features=1, subtract_last=False, affine=False, rngs=nnx.Rngs(0))
    x = _x()
    z, loc, scale = revin.norm(x)
...
# ---- STAD (series-core fusion) ----------------------------------------------

def test_stad_shape_preserved():
    B, C, hidden, d_core = 2, 5, 32, 16
    stad = STAD(hidden_size=hidden, d_core=d_core, rngs=nnx.Rngs(0))
...
def test_stad_deterministic_is_idempotent():
    stad = STAD(hidden_size=16, d_core=8, rngs=nnx.Rngs(0))
    ...
    np.testing.assert_allclose(o1, o2, rtol=1e-6)   # eval core is deterministic


def test_stad_train_stochastic_pooling_differs_across_calls():
    # Multinomial pooling draws a fresh key each forward -> two train-mode passes
    # over the same input differ (the whole point of stochastic pooling).
    stad = STAD(hidden_size=16, d_core=8, rngs=nnx.Rngs(0))
...
def test_stad_single_series_pooling_agrees_across_modes():
    # With one series (C=1) softmax over the series axis is 1 and the multinomial
    # can only pick index 0, so the aggregated core — and hence the output — is the
    # same in train and eval mode. Guards the pooling/gather axis wiring.
    stad = STAD(hidden_size=16, d_core=8, rngs=nnx.Rngs(0))
...
def test_stad_core_is_shared_across_series_in_gen2_space():
    # The aggregated core is broadcast to every series before the fusion MLP; a
    # constant-across-series input therefore produces a constant-across-series
    # output (all series see the identical dispatched core).
    stad = STAD(hidden_size=16, d_core=8, rngs=nnx.Rngs(0))
    row = np.random.RandomState(4).randn(2, 1, 16)
    x = jnp.asarray(np.repeat(row, 5, axis=1), dtype=jnp.float32)  # [2, 5, 16], series identical
...
def test_encoder_layer_shape():
    B, T, hidden, d_core = 2, 7, 32, 16
    layer = TransEncoderLayer(hidden_size=hidden, d_core=d_core, d_ff=64,
...
def test_encoder_stack_shape_and_layer_count():
    B, T, hidden, d_core = 2, 7, 32, 16
    enc = TransEncoder(e_layers=3, hidden_size=hidden, d_core=d_core, d_ff=64,
                       dropout=0.0, rngs=nnx.Rngs(0))
    x = jnp.ones((B, T, hidden), dtype=jnp.float32)
...
def _net(h=24, input_size=72, hidden=32, d_core=16, layers=2):
    return SOFTSNet(
...
def test_net_is_n_generic_multivariate():
    ...
    # SOFTS wrapper fixes N=1; this guards the future multivariate path (and the
    # STAD pooling over the series axis with N > 1).
    net = _net()
...
def test_net_use_norm_false_still_forwards():
    net = SOFTSNet(h=12, input_size=36, hidden_size=16, d_core=8,
...
def test_net_d_core_independent_of_hidden_size():
    ...
    for d_core in (3, 30, 100):
        net = SOFTSNet(h=4, input_size=12, hidden_size=30, d_core=d_core, e_layers=1,
...
def _train_net(h=12, input_size=36, hidden=16, d_core=8, layers=1):
    return SOFTSNet(
        h=h, input_size=input_size, hidden_size=hidden, d_core=d_core,
...
def test_train_deterministic_with_same_seed():
    # Same rng seed drives both param init AND STAD's pooling key stream, so two
    # identical runs must match bitwise despite the stochastic pooling.
    y = _make_y()
...
def _tiny():
    return SOFTS(h=12, input_size=36, hidden_size=16, d_core=8, e_layers=1,
                 d_ff=32, max_steps=20, windows_batch_size=64, random_seed=0)


def test_softs_inherits_baseforecaster():
    assert issubclass(SOFTS, BaseForecaster)
...
def test_model_beats_naive_last_value_on_easy_signal():
    ...
    # projection/STAD wiring that still passes shape + determinism tests.
    n = 200
    y = jnp.asarray(np.sin(np.arange(n) / 5.0), dtype=jnp.float32)
    m = SOFTS(h=12, input_size=36, hidden_size=32, d_core=32, e_layers=2,
...
def test_loss_custom_callable_trains():
    ...
    m = SOFTS(h=12, input_size=36, hidden_size=16, d_core=8, e_layers=1,
...
def test_softs_importable_from_models_namespace():
    from chronax.models import SOFTS as S
    assert S is SOFTS
...
def test_h_equals_one():
    m = SOFTS(h=1, input_size=12, hidden_size=8, d_core=4, e_layers=1,
...
def test_predict_with_level_returns_interval_keys():
    m = SOFTS(h=4, input_size=12, hidden_size=8, d_core=4, e_layers=1,
...
def test_use_boxcox_off_keeps_lambda_none_and_matches_plain():
    ...
    m_off = SOFTS(h=12, input_size=36, hidden_size=16, d_core=8, e_layers=1,
                  d_ff=32, max_steps=20, windows_batch_size=64, random_seed=0).fit(y)
    assert m_off._bc_lambda is None
    plain = SOFTS(h=12, input_size=36, hidden_size=16, d_core=8, e_layers=1,
                  d_ff=32, max_steps=20, windows_batch_size=64, random_seed=0,
...
def test_use_boxcox_forecast_fitted_inverts_to_original_scale():
    y = jnp.asarray(np.exp(np.linspace(0, 3, 200)) + 1.0, dtype=jnp.float32)
    res = SOFTS(h=12, input_size=36, hidden_size=16, d_core=8, e_layers=1,
                d_ff=32, max_steps=20, windows_batch_size=64, random_seed=0,

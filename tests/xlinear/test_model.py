"""Tests for chronax.models.xlinear.xlinear_model — BaseForecaster conformance."""
import pickle
...
def _tiny(**kw):
    return XLinear(h=12, input_size=36, hidden_size=16, temporal_ff=8, channel_ff=4,
                   max_steps=20, windows_batch_size=32, random_seed=0, **kw)


def test_xlinear_inherits_baseforecaster():
    assert issubclass(XLinear, BaseForecaster)
...
def test_unknown_scaler_raises_at_fit():
    with pytest.raises(ValueError, match="Unknown scaler"):
        XLinear(h=12, input_size=36, hidden_size=16, temporal_ff=8, channel_ff=4, max_steps=2, windows_batch_size=8, random_seed=0, scaler="nope").fit(_make_y())
...
def test_model_beats_naive_on_easy_signal():
    ...
    m = XLinear(h=12, input_size=36, hidden_size=16, temporal_ff=8, channel_ff=4,
                max_steps=200, windows_batch_size=32, random_seed=0,
...
def test_h_equals_one():
    m = XLinear(h=1, input_size=12, hidden_size=8, temporal_ff=4, channel_ff=2, max_steps=5, windows_batch_size=16, random_seed=0)
    m.fit(_make_y(60))
...
def test_predict_with_level_returns_interval_keys():
    m = XLinear(h=4, input_size=12, hidden_size=8, temporal_ff=4, channel_ff=2, max_steps=2, windows_batch_size=16, random_seed=0)
    m.fit(_make_y(80))
...
def test_architecture_knobs_thread_to_predictions():
    y = _make_y()
    base = dict(h=12, input_size=36, max_steps=20, windows_batch_size=32, random_seed=0)
    p_ref = np.asarray(XLinear(hidden_size=16, temporal_ff=8, channel_ff=4, **base).fit(y).predict(h=12)["mean"])
    for kw in (dict(hidden_size=4, temporal_ff=8, channel_ff=4),
               dict(hidden_size=16, temporal_ff=32, channel_ff=4),
               dict(hidden_size=16, temporal_ff=8, channel_ff=2)):
        p = np.asarray(XLinear(**kw, **base).fit(y).predict(h=12)["mean"])
        assert not np.allclose(p, p_ref), kw


def test_pickle_round_trip_nondefault_knobs():
    m = XLinear(h=12, input_size=36, hidden_size=8, temporal_ff=12, channel_ff=6,
                max_steps=20, windows_batch_size=32, random_seed=0).fit(_make_y())
    before = np.asarray(m.predict(h=12)["mean"])
    m2 = pickle.loads(pickle.dumps(m))
    assert (m2.model_.hidden_size, m2.model_.temporal_ff, m2.model_.channel_ff) == (8, 12, 6)
    np.testing.assert_allclose(np.asarray(m2.predict(h=12)["mean"]), before, rtol=1e-5, atol=1e-5)


def test_use_norm_false_fits_and_differs():
    y = _make_y()
    a = np.asarray(_tiny().fit(y).predict(h=12)["mean"])
    b = np.asarray(_tiny(use_norm=False).fit(y).predict(h=12)["mean"])
    assert np.all(np.isfinite(b)) and not np.allclose(a, b)


def test_fitted_values_closed_form_use_norm():
    from chronax.models.xlinear.xlinear_module import _revin
    y = _make_y(60)
    m = _tiny().fit(y)
